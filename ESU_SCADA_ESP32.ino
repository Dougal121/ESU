...
//      display.drawRect(0,33,128,11);
    ...
    if ( esui.bOnOffState ){   // Inverter and Incommer sequencer
      if (esuc.bMasterEnable && ( year() > 2019 ) && ( now() > ghks.AutoOff_t )){
        if ( esui.bIncomer ) {
    ...
    if (( lcct == lsst ) && ( esuc.bSolarEnable )  ){
      esui.iOnTime = esuc.iOnTimeSunset ;
    }
    if (( lcct == ( lsrt - esuc.iOnTimeSunrise )) && ( esuc.bSolarEnable )  ){
      esui.iOnTime =  esuc.iOnTimeSunrise ;
    }    
    
    if (( esui.iOnTime > 0 ) && esui.bVoltageEnable ){  // hold on
      esui.bOnOffState = true ;
    }
